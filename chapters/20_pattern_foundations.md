...
1. $f=g=0$ なら反応項がないので，
   ...
   \qquad
   \frac{\partial v}{\partial t}=D_v\Delta v
   ```

   となる．高い場所から低い場所へ広がり，空間的な凹凸は小さくなる．拡散だけなので，一般には模様を作るよりも模様をならす方向に働く．

2. $D_u=D_v=0$ なら，格子点どうしは情報を交換しない．さらに全格子点の初期値が同じなら，どの格子点も同じ反応ODEを同じ初期値から解くため，時間が進んでも全点が同じ値を保つ．空間的な差がないので，模様は生じにくい．
...
3. 5点ラプラシアンは
   ...
   である．中央が1，上下左右が0なら分子は $0+0+0+0-4=-4$ なので，符号は負である．拡散だけなら中央の値は下がる．

4. 周期境界では，右端と左端，上端と下端がつながっているとみなす．そのため，ヘビの頭・尾・腹側との境界，表面の曲率や太さの変化，実際の鱗や模様が端で途切れる効果を無視している．長い胴体の一部を切り出し，同じ構造が繰り返す近似として使っている，と説明できる．

5. seedを変えても，面積比，斑点数，代表的な波長，縞の方向分布などの統計的特徴は大きく保たれることが望ましい．一方，斑点の正確な位置，局所的なゆらぎ，どの斑点がどこで分裂するか，といった細部は変わってよい．
````
 -->
