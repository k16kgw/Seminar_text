...
:::{note} 第2回（50分）の位置づけ
本章と[第4章](./04_reaction_diffusion_snake.md)を学生Bが1回で担当する。第1回の熱方程式から**拡散項**を引き継ぎ、局所反応を加えると何が変わるかを中心にする。0–27分で反応拡散モデル、27–44分でパターン生成と特徴量による比較を示す。
:::
...
[共通準備](./00_pde_foundations.md)の1成分拡散方程式は凹凸をならすだけだった。本章では、各格子点で進むODE $\dot u=f(u,v)$, $\dot v=g(u,v)$ と、隣の格子点と混ざる拡散を足す。つまり「既習の局所ODE＋共通準備の空間結合」である。
