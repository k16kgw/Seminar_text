...
<!-- 
````{dropdown} 解答例
1. 本章の規約では，$A_{ij}$ は頂点 $j$ から頂点 $i$ へ入る影響である．したがって $A_{31}=-0.4$ は，頂点1から頂点3へ重み $-0.4$ の矢印があることを表す．言葉では，頂点1の状態が頂点3の変化率へ負の影響を与える，と読める．
...
2. 上の行列
   ...
   では，第2行が $(0,0,1.0)$ である．したがって

   ```{math}
   (A\mathbf{x})_2=0x_1+0x_2+1.0x_3=x_3
   ```

   となる．頂点2は頂点3から正の影響を受ける．

3. $\alpha$ は自己減衰の強さである．$\alpha$ だけを大きくすると，各状態は単独ではより強く0へ戻ろうとする．系行列 $-\alpha I+\beta A$ の固有値は全体に負の方向へずれるので，一般には収束しやすくなる．ただし，強いネットワーク結合や非線形項を加えたモデルでは別途確認が必要である．
...
4. 対応する固有値は
   ...
   である．符号は負なので，この固有方向の小さなずれは減衰する．

5. 人物ごとの状態では，変数は $x_i(t)$ のように頂点1つに対して1つ置く．添字 $i$ は人物や頂点を表す．人物間の関係を状態にする場合は，変数は $x_{ij}(t)$ のように2つの添字を持ち，人物 $i$ から人物 $j$ への有向関係を表す．後者は最大で $n(n-1)$ 個になり，頂点状態モデルより変数数が増えやすい．
````
 -->
