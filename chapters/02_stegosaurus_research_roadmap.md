...
| 時間 | 作業 | 50分後に残すもの |
| ---: | --- | --- |
| 0～6分 | 研究の問いを1文へ狭める | 「固定する量」「変える量」「測る量」 |
| 6～13分 | 長方形基準計算の確認項目を選ぶ | 基準版の合格条件 |
| 13～21分 | 最初に追加する形状を紙に描く | 輪郭式または座標定義の案 |
| 21～29分 | 公平な比較条件を1つ選ぶ | 同高さ・同面積などの選択理由 |
| 29～37分 | 2形状比較の手順を書く | 変更しないパラメータ表 |
| 37～44分 | 主評価指標を1つ選ぶ | 指標の式・単位・意味 |
| 44～48分 | 数値的・資料的リスクを挙げる | 失敗時の縮小案 |
| 48～50分 | 次回までの作業を決める | 実装1つ、確認1つ |

この章は通読するだけでなく、各段階の欄を自分の研究計画で埋める。50分で複数形状の計算を始める必要はない。**最初の2形状比較を再現可能にする設計書**が成果物である。
...
空欄を残した場合は、コードを書く前にその空欄が計算条件へ影響するかを確認する。
