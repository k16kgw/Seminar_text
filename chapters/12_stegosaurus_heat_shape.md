...
各項の単位を確認すると，$kt\Delta T$ と $2h(T-T_\mathrm{air})$ はともに単位面積当たりの熱移動率に対応する．$\Delta T$ は温度差ではなく，ここではラプラシアンを温度 $T$ へ作用させた記号である．記号 $\Delta$ と温度差を表す記号が同じに見えるため，文脈に注意する．式の第1項は板面内の熱伝導，第2項は前面・背面からの熱損失である．

板内で周囲より温度が高い点ではラプラシアンが負になり，面内伝導によって熱が外側へ流れる．同時に，外気より温度が高ければ対流項でも熱を失う．定常状態では，根元から供給される熱がこの2種類の流れを補い，全体の温度分布が時間変化しなくなる．
...
内部セルは通常4つの隣接セルを持つが，外周セルでは形状外側に隣接セルが存在しない．その辺では伝導項を置く代わりに外周対流項を加える．根元セルでは熱収支式を温度固定式 $T_i=T_b$ に置き換える．したがって，内部，外周，根元の3種類を分けて行列を組み立てる．

1つの内部セルについて式を展開すると，対角成分には隣接伝導コンダクタンスの合計と対流コンダクタンスが入り，隣接セルに対応する非対角成分には $-G_\mathrm{cond}$ が入る．行列の1行を見て，どのセルとの熱交換を表しているか図上で指差せるようにする．
...
と無次元化して記録する．絶対差だけでは，放熱量の大きい条件と小さい条件を公平に比較できない．格子幅ごとに $Q_\mathrm{base}$，$Q_\mathrm{out}$，$\varepsilon_Q$ を表へまとめ，誤差が減少するか確認する．
