...
| 時間 | 学習内容 | 到達確認 |
| ---: | --- | --- |
| 0～7分 | 頂点・辺・状態を区別する | 3つを別々に説明する |
| 7～16分 | 隣接行列の添字規約 | $A_{ij}$ の矢印方向を書く |
| 16～24分 | 行列から有向グラフへ | 全3辺を照合する |
| 24～33分 | 結合ODEと固有値 | $M=-\alpha I+\beta A$ を読む |
| 33～40分 | 3頂点の時系列 | 影響が伝わる順序を説明する |
| 40～46分 | 結合強度と安定性 | 最大固有値実部の0交差を読む |
| 46～50分 | チェックポイント | 研究対象の意味づけ前の基礎を確認する |
...
![3頂点の有向重み付きネットワーク。](../assets/figures/notebook/08_network_graph.png)

本章では $A_{ij}$ を「頂点 $j$ から頂点 $i$ への影響」と定義する。したがって $A_{1,2}=0.8$ は node 2 から node 1 への矢印、$A_{3,1}=0.5$ は node 1 から node 3 への矢印、$A_{2,3}=1.0$ は node 3 から node 2 への矢印になる。

線の太さと数値は辺重みを表す。頂点の位置は円形レイアウトで見やすく置いただけで、人物間の物理的距離や関係の近さを意味しない。矢印を逆に描くと以後の時系列解釈もすべて逆になるため、非対称な行列で照合することが重要である。

### 影響が伝わる時系列

![node 1だけを非ゼロにした初期状態からの3頂点時系列。](../assets/figures/notebook/08_network_timeseries.png)
...
![結合強度betaと係数行列の最大固有値実部。](../assets/figures/notebook/08_network_stability.png)
