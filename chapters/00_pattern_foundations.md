...
:::{note} この資料の位置づけ
ヘビ模様と反応拡散を学ぶ前の準備章である。Strogatzで学んだ2変数ODEを、空間の各点に置くところから始める。[PDE入門](./00_pde_foundations.md)の「場・ラプラシアン・差分法」を先に読むと理解しやすい。
:::
...
| 時間 | 学習内容 | 到達確認 |
| ---: | --- | --- |
| 0～8分 | 2変数ODEの復習 | 反応だけの状態変化を書ける |
| 8～17分 | 変数を場 $u(x,y,t),v(x,y,t)$ へ拡張 | 1時刻の状態が画像になると説明する |
| 17～27分 | 反応項と拡散項を分けて読む | 片方を切った極限を説明する |
| 27～35分 | 2次元ラプラシアンと境界条件 | 5点差分を中央と近傍に分ける |
| 35～42分 | 初期擾乱・seed・対称性の破れ | 一様初期値だけでは模様が出にくい理由を述べる |
| 42～47分 | 模様を記述する基本量 | 面積比・個数・波長・方向を区別する |
| 47～50分 | 確認問題 | 後続章へ進めるか自己判定する |
...
[反応拡散の基礎：パターン生成](./03_reaction_diffusion_basic.md)で具体的なGray--Scottモデルを実行し、その後[模様解析の基礎](./04_reaction_diffusion_snake.md)で画像特徴量を個別に検証する。
