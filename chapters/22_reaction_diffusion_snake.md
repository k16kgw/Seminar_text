...
:::{note} 学習経路での位置づけ
本章では研究技能を練習する．ヘビ写真と反応拡散パターンの適合はまだ行わない．人工的な斑点・縞画像を使い，{abbr}`二値化(画素を閾値によって前景と背景の2値へ分ける処理)`，{abbr}`面積比(画像全体に占める前景画素の割合)`，{abbr}`連結成分数(隣接してつながる前景領域の個数)`，{abbr}`特徴波長(模様で支配的に繰り返される空間間隔)`を1つずつ測る．
:::
...
これらは[卒研ロードマップ](./23_snake_research_roadmap.md)で学生が設計する．
...
[卒研ロードマップ：生成模様と実画像を比較する](./23_snake_research_roadmap.md)では，対象画像の選定，前処理，特徴量選択，損失関数，パラメータ探索を小さな段階に分ける．完成した適合結果は示さない．
