...
| 時間 | 学習内容 | 到達確認 |
| ---: | --- | --- |
| 0～6分 | 1次元から2次元へ広げる理由 | $T(x)$ と $T(x,y)$ の違いを説明する |
| 6～16分 | 単一長方形薄板の仮定 | 今回変えない条件を列挙する |
| 16～27分 | 薄板方程式とセル熱収支 | 伝導・面対流・縁対流を区別する |
| 27～36分 | 温度場と局所放熱密度の図 | 色、等温線、2つの図の違いを読む |
| 36～43分 | エネルギー収支と格子収束 | $Q_\mathrm{base}$ と $Q_\mathrm{out}$ を照合する |
| 43～48分 | $h$ 感度図を読む | 温度低下と放熱増加を同時に説明する |
| 48～50分 | チェックポイント | 形状追加前の検証が十分か判断する |

本章の50分では新しい形状を作らない。単一形状の計算を説明・検証できることが到達点である。
...
![薄板内部、固定温度の根元、対流する外周を色分けした境界条件の模式図。](../assets/figures/concepts/plate_boundary_conditions.svg)

形状マスクの内側だけに熱伝導方程式を置く。赤い根元は固定温度、空気に触れる側面と先端は対流境界であり、形状の外側は未知温度を解く領域ではない。
...
![単一長方形薄板の温度場と局所放熱密度](../assets/figures/notebook/02_single_plate_temperature.png)
...
この図から確認すべきことは「上部が暗いから不要」という結論ではない。上部へ届く前に側面から熱が逃げた結果として冷えている。形状の有効性を判断するには、局所色だけでなく、全セルと外周を積分した $Q_\mathrm{out}$ が必要になる。
...
![対流熱伝達率hを変えた中心線温度と総放熱量。](../assets/figures/notebook/02_plate_h_sensitivity.png)

横軸は高さ、縦軸は板中央線の温度である。すべての曲線は根元の $40\,{}^\circ\mathrm{C}$ から始まる。$h$ が大きい緑線ほど急速に冷え、先端温度は外気温へ近づく。一方、凡例の総放熱量 $Q$ は $h=10,25,50$ の順に増えている。

「板が冷たい」と「放熱していない」は同じではない。$h$ が大きいと熱を強く奪うため板温度は低くなるが、単位時間に外へ移る熱量は増える。この一見逆向きの変化を説明できることが、温度図と放熱量を併記する理由である。
