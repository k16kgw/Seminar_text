...
# 共通準備：ODEからPDEへ――拡散方程式入門

:::{important} この資料の位置づけ
偏微分方程式を初めて学ぶ人のための**全員必修の事前学習**である。輪講担当には含めない。本文を読み、対応Notebookを一度実行してから第1回へ進むこと。
:::
...
この章の到達点は、PDEの一般論を網羅することではない。第1回の熱方程式と第2回の反応拡散方程式を、式の意味と計算手順の両方から読めるようになることである。
...
ここでPDEが突然ODEに変わったように見えるが、空間だけの未知関数を求める常微分方程式になったのである。第1回の1次元フィン方程式は、この定常化と細長い形状の仮定によって得られる。
...
2次元の正方格子と5点差分では、対応する条件は概ね $D\Delta t/\Delta x^2\le1/4$ になる。第2回の反応拡散で再利用する。
...
第1回の[ステゴサウルス背板の放熱 I](./01_stegosaurus_heat_basic.md)では、温度場、定常化、Dirichlet条件、Neumann条件、Robin条件を具体的な放熱問題で使う。第2回の[ヘビ模様の反応拡散 I](./03_reaction_diffusion_basic.md)では、2次元ラプラシアンと数値安定性を再利用する。
