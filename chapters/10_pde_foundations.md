...
差分値で符号を確認してみる．中央が $u_i=10$，左右が $u_{i-1}=8$, $u_{i+1}=8$ なら，分子は $8-2\times10+8=-4$ となり，2階微分は負である．拡散方程式では次時刻の中央値が下がる．反対に中央が6，左右が8なら分子は4であり，中央値は上がる．この小例を頭に置くと，ラプラシアンの符号を式の暗記ではなく，周囲平均との差として判断できる．
...
が保存則である．{abbr}`流束(単位面積を単位時間に通過する量)` $J$ が勾配の反対向きに流れるというFick/Fourier型の法則
