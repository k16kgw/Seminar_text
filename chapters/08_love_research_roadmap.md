...
| 時間 | 作業 | 50分後に残すもの |
| ---: | --- | --- |
| 0～7分 | 対象作品・人物・場面範囲を限定 | 再収集可能な対象定義 |
| 7～15分 | 1時刻の単位を決める | 話・場面・一定時間の選択理由 |
| 15～23分 | 状態変数を操作的に定義 | 正負、尺度、不明値の規則 |
| 23～30分 | 少量アノテーションを設計 | 3～5場面の試行表 |
| 30～37分 | 2人基準モデルを選ぶ | 状態数と係数数 |
| 37～43分 | 基準モデルの残差候補を考える | 拡張を必要とする観測例 |
| 43～47分 | 推定用・検証用の分割を決める | 場面分割案 |
| 47～50分 | 次の作業を限定 | アノテーション基準の試行だけ |

この50分では三角関係の式を完成させない。データとして何を観測できるかを先に定め、2人基準モデルから始められる研究設計を成果物とする。
...
最初は主要な2人だけを選び、[関係モデル基礎の線形モデル](./07_love_dynamics_basic.md)またはさらに単純な離散モデルで説明できる範囲を確認する。
...
- 状態変数は第三者が同じ基準で評価できるか。
- 追加した係数は具体的なデータに支えられているか。
- モデルを複雑にする前に基準モデルの失敗を示したか。
- 推定に使ったデータと評価に使ったデータを分けたか。
- 作品解釈と数理モデルの予測を区別して書いているか。
