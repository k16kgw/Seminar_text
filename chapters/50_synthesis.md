...
各テーマの基礎章と研究技能章から，自分の卒業研究で再利用できる{abbr}`設計原理(異なる研究対象にも再利用できるモデル化や検証の基本方針)`を取り出す．自分のテーマのロードマップを具体化するとき，必要に応じて下の比較表と研究計画シートへ戻る．
...
他テーマから方法を借りるとは，対象や数式をそのまま移すことではない．たとえば乗降研究で学んだ乱数seedの反復は，反応拡散模様の初期擾乱にも使える．同じ反応パラメータでseedだけを変え，特徴波長や面積比の分布を調べれば，1枚の模様に依存しない比較になる．共通する問題構造を見つけ，検証手順を移す．

同様に，放熱研究の格子収束は，反応拡散の空間刻みや乗降モデルのセル幅を変えた感度確認へ移せる．ネットワーク研究の行列と矢印の照合は，どのテーマでもコード中の結合関係を小例で確かめる考え方として使える．
...
借りる知見を選ぶときは，自分の研究で最も不確かな部分を探す．比較条件が曖昧なら放熱，見た目しか評価できていないなら反応拡散，乱数や更新順序の影響が大きいなら乗降，影響関係の向きが複雑ならネットワークの章へ戻る．方法を追加した後は，研究計画のどの欄が具体化されたかを記録する．
...
各欄は互いに独立ではない．問いを変更すれば状態変数と評価指標も見直し，境界条件を変更すれば検証方法も見直す．1つの欄を更新したときは，関連する欄へ矛盾が生じていないか確認する．

最初の図は，計算後に考える飾りではなく，研究設計を検査する道具である．横軸に変える量，縦軸に評価指標，線や色に比較条件を書けない場合は，問いか比較条件がまだ曖昧である．仮の図を紙に描いてから計算条件を決める．
...
- 他テーマの発表から得た知見を使い，自分の研究計画を実際に1か所変更したか．
